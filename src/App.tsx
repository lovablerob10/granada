        ...
        <div className="min-h-screen bg-granada-black">
            ...
            <nav className="fixed top-0 w-full z-50 bg-black/40 backdrop-blur-xl border-b border-white/5 shadow-2xl transition-all duration-300">
                <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
                    <div className="flex items-center overflow-visible">
                        <img src="/logo-completo.png" alt="Club Granada" className="h-[75px] w-auto drop-shadow-lg" />
                    </div>
                    ...
                    <Link to="/dashboard" className="btn-premium py-2 px-6 rounded-none text-xs flex items-center justify-center">Login</Link>
                </div>
            </nav>
            ...
            <section className="relative h-screen w-full overflow-hidden bg-black">
                ...
                <div className="relative z-10 h-full flex flex-col justify-end items-center pb-24 px-4">
                    ...
                    >
                        <p className="text-2xl md:text-5xl max-w-4xl mx-auto font-bebas tracking-[0.2em] uppercase leading-tight drop-shadow-[0_0_20px_rgba(212,17,17,0.5)] animate-text-shimmer">
                    ...
                    >
                        <button className="btn-premium text-lg px-16 py-5 rounded-none animate-shine shadow-[0_0_30px_rgba(212,17,17,0.4)]">
                            Seja Sócio Granada
                        </button>
                    </motion.div>
                </div>
            ...
            {/* Plans Section */}
            <section id="planos" className="py-24 px-6 relative overflow-hidden bg-black">
                ...
                <div className="max-w-7xl mx-auto relative z-10">
                    <div className="text-center mb-16">
                        <h2 className="text-5xl md:text-7xl mb-4 text-white uppercase">FAÇA PARTE DA <span className="text-granada-red">EXPLOSÃO</span></h2>
                        <p className="text-gray-500 uppercase tracking-widest text-sm">Planos pensados para os verdadeiros apaixonados pelo Granada.</p>
                    </div>
                        ...
                        {plans.map((plan, i) => {
                            ...
                            return (
                                <motion.div
                                    key={i}
                                    whileHover={{ y: -5 }}
                                    className={`card-premium p-8 relative overflow-hidden ${plan.featured ? 'border-granada-red' : ''}`}
                                >
                                    {plan.featured && (
                                    ...
                                    <h3 className="text-4xl mb-2 text-white">{plan.name}</h3>
                                    ...
                                    <ul className="space-y-4 mb-10 text-gray-400 text-sm border-t border-white/5 pt-8">
                                        {plan.benefits.map((benefit, j) => (
                                            <li key={j} className="flex gap-2"><ChevronRight size={16} className="text-granada-red" /> {benefit}</li>
                                        ))}
                                    </ul>
            ...
            <section className="py-12 bg-black border-t border-white/5 relative overflow-hidden">
                ...
                <div className="flex animate-scroll-x whitespace-nowrap gap-20">
                    {[...sponsors, ...sponsors].map((sponsor, i) => (
                        <div
                            key={i}
                            className={`sponsor-item flex-shrink-0 font-bebas text-3xl md:text-5xl ${sponsor.red ? 'text-granada-red' : 'text-white'} tracking-[0.2em] px-10`}
                        >
                            {sponsor.name}
                        </div>
                    ))}
            ...
            {/* Final CTA */}
            <section className="py-24 px-6 bg-black text-center overflow-hidden relative border-t border-white/5">
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[600px] bg-granada-red/10 rounded-full blur-[150px] pointer-events-none" />
                <motion.div
                    initial={{ opacity: 0 }}
                    whileInView={{ opacity: 1 }}
                    viewport={{ once: true }}
                >
                    <h2 className="text-5xl md:text-8xl mb-8 text-white uppercase tracking-tighter">SINTA A EMOÇÃO, <span className="text-granada-red">VIVA A EXPLOSÃO</span></h2>
                    <p className="text-xl text-gray-500 max-w-2xl mx-auto mb-12 uppercase tracking-wide">Junte-se ao Clube Granada e faça história.</p>
                    <button className="btn-premium text-xl px-16 py-5 rounded-none">
                        SEJA SÓCIO AGORA
                    </button>
                </motion.div>
            </section>
            ...
            {/* Footer */}
            <footer className="py-12 border-t border-white/10 px-6 bg-black">
